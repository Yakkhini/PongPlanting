...
use crate::{appstate, ball, board, brick};

pub struct BallBrickCollisionEvent {
    pub brick: Entity,
}

pub struct BallBoardCollisionEvent;
...
fn ball_brick_collision_event_writer(
    ...
    mut event_writer: EventWriter<BallBoardCollisionEvent>,
    ...
    for event in collide_events.iter() {
        if let CollisionEvent::Stopped(data1, data2) = event {
            if (board_entity == data1.rigid_body_entity()
                && data2.rigid_body_entity() == ball_entity)
                || (board_entity == data2.rigid_body_entity()
                    && data1.rigid_body_entity() == ball_entity)
            {
                event_writer.send(BallBoardCollisionEvent);
            }
        }
    }
}

fn ball_board_collision_event_writer(
    mut collide_events: EventReader<CollisionEvent>,
...
impl Plugin for CollisionPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_event::<BallBoardCollisionEvent>();
        app.add_system_set(
        ...
        app.add_system_set(
            SystemSet::on_update(appstate::AppState::InGame)
                ...
                .with_system(ball_board_collision_event_writer),
        );
    }
}
